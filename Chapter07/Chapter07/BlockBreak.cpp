...
int main4() {
	srand(time(NULL));
	RenderWindow window(VideoMode(300, 480), "ARKANVOID");
	window.setFramerateLimit(60);
	
	Texture t1, t2, t3, t4;
	t1.loadFromFile("image/block.jpg");
	t2.loadFromFile("image/backgroundblack.png");
	t3.loadFromFile("image/ball.png");
	t4.loadFromFile("image/paddle.jpg");

	Sprite background(t2), ball(t3), paddle(t4);
	paddle.setPosition(150, 440);
	...
	while (window.isOpen())
	{
		...
		if (ball_pos.x < 0 || ball_pos.x>250)
			dx = -dx;
		if (ball_pos.y < 0 || ball_pos.y>480)
			dy = -dy;

		//공이 화면 하단을 벗어나면 게임이 종료됨
		if (ball_pos.y>=480 ){
			break;
		}
